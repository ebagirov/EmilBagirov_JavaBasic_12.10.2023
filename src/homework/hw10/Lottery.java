...
public class Lottery {
    public static void main(String[] args) {
        int[] ticket = generateCombo();
        int[] player = generateCombo();
        int[] sortTicket = sortArray(ticket);
        int[] sortPlayer = sortArray(player);

        System.out.println("\n" + "Ticket: " + Arrays.toString(ticket));
        System.out.println("Player: " + Arrays.toString(player) + "\n");
        System.out.println("Ticket sort: " + Arrays.toString(sortTicket));
        System.out.println("Player sort: " + Arrays.toString(sortPlayer) + "\n");
//        System.out.println("Matched numbers: " + results(ticket, player));
    }

    public static int[] generateCombo() {
        int[] combo = new int[7];
        for (int i = 0; i < combo.length; i++) {
            combo[i] = ThreadLocalRandom.current().nextInt(10);
        }
        return combo;
    }

    public static int[] sortArray(int[] ticket) {
//        int[] sort = new int[7];
